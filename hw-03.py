...
def get_numbers_ticket():
    range_x = range(1,1001)
    lotary = sorted(random.sample(range_x,5))
    attempts = 5
    while attempts > 0:
        try:
            user_input = input("Введіть 5 чисел від 1 до 1000 через пробіл: ")
            user_numbers = list(map(int, user_input.strip().split()))
            if len(set(user_numbers)) != 5:
                print("❌ Числа повинні бути унікальні та потрібно ввести рівно 5 чисел.")
                attempts -= 1
                continue
            if not all(1 <= num <= 1000 for num in user_numbers):
                print("❌ Усі числа мають бути в діапазоні від 1 до 1000.")
                attempts -= 1
                continue
            user_numbers.sort()
